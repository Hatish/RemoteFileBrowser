...
namespace RemoteFileBrowser
{
  public class FileBrowserHelper
  {
    ...
    void copier_OnProgressChanged(double Persentage, long progress, string source, ref bool Cancel)
    {
      ...
      if (perc != copyPercentage && perc % 5 == 0)
      {
        ...
        {
          Data = new CopyDataUpdate() { Progress = perc, Destination = destination, jobTotal = jobSize, jobProgress = (jobProgress + progress), Source = source },
          id = 0,
  ...
  public delegate void ProgressChangeDelegate(double Persentage, long bytes, string fileName, ref bool Cancel);
      ...
      using (FileStream source = new FileStream(SourceFilePath, FileMode.Open, FileAccess.Read))
      {
        ...
        using (FileStream dest = new FileStream(DestFilePath, FileMode.CreateNew, FileAccess.Write))
        {
          ...
          while ((currentBlockSize = source.Read(buffer, 0, buffer.Length)) > 0)
          {
            ...
            cancelFlag = false;
            OnProgressChanged(persentage, totalBytes, this.SourceFilePath, ref cancelFlag);
